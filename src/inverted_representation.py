...
class InvertedRepresentation():
    ...
    def get_output_from_specific_layer(self, x, layer_id):
        ...
        print(x.size())
        layer_output = None
        for index, layer in enumerate([self.model.module.conv1, self.model.module.layer1, self.model.module.layer2, self.model.module.layer3, self.model.module.layer4]):
            x = layer(x)
            if str(index) == str(layer_id):
                layer_output = x[0]
                break
        return layer_output.cpu()
...
if __name__ == '__main__':
    # Get params
    target_example = 0  # bear
    (original_image, prep_img, target_class, file_name_to_export) = get_example_params(target_example)
    pretrained_model = torch.load('../res/net_C.pkl')
    inverted_representation = InvertedRepresentation(pretrained_model)
    image_size = 224  # width & height
    target_layer = 4
    prep_img = prep_img[:,:,:224,:224]
    print(prep_img.size())
